...
/// din GDCVault/DataMover (vezi CLAUDE.md Partea 1, Regula 20).
///
/// WARNING: pasul de instalare (promptul de parolă admin) NU poate fi
/// verificat automat de Claude — cere interacțiune fizică reală a userului
/// cu fereastra de sistem. Verificat automat doar descărcarea (URL valid,
/// HTTP 200, fișier scris integral pe disc). Instalarea efectivă TREBUIE
/// confirmată manual, o dată, de Cristi, înainte să considerăm fluxul
/// complet dovedit.
enum SelfUpdater {
    ...
    static func downloadAndInstall(pkgURL: URL, version: String) async {
        ...
        do {
            let tempDir = FileManager.default.temporaryDirectory
                .appendingPathComponent("cgconvertor-update-\(UUID().uuidString)", isDirectory: true)
            try FileManager.default.createDirectory(at: tempDir, withIntermediateDirectories: true)
            let pkgPath = tempDir.appendingPathComponent("CGConvertor-\(version).pkg")
    ...
    private static func runInstaller(pkgPath: URL, tempDir: URL) throws {
        let logPath = tempDir.appendingPathComponent("cgconvertor_update.log")
        let scriptPath = tempDir.appendingPathComponent("cgconvertor_update.sh")
    ...
    private static let releasesPageURLForFallback = URL(string: "https://github.com/gordasgdc/CGConvertor/releases/latest")!
...
final class UpdateProgressWindow {
    ...
    init(version: String) {
        ...
        let titleLabel = NSTextField(labelWithString: "CG Convertor \(version)")
