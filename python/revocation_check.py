...
1:1 al `gdc-production-manager/backend/revocation_check.py` (identic ca
stack, pur `urllib`/`threading`, fără nicio dependință de framework)
pentru CG Convertor. Adaugă un "kill-switch" pe care Cristi îl poate
declanșa manual din Furnizor, FĂRĂ să schimbe deloc formatul criptografic
al codurilor existente.
...
răspuns "false" ulterior).

Folosește apelul RPC `is_license_revoked(machine_id, product_id)` din
Supabase — NICIODATĂ un SELECT direct pe tabel: RLS blochează complet
accesul direct cu cheia anon.
"""
