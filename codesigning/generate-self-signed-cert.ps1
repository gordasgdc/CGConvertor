...
#      - cgconvertor-selfsign.pfx  (PRIVAT, cu cheie - NU se distribuie,
#        NU se comite in git, NU se lipeste in chat/conversatie)
#      - cgconvertor-selfsign.cer  (PUBLIC, fara cheie - se distribuie
...
$subject = "CN=CG Convertor (Self-Signed, testare interna)"
$pfxPath = Join-Path $PSScriptRoot "cgconvertor-selfsign.pfx"
$cerPath = Join-Path $PSScriptRoot "cgconvertor-selfsign.cer"
...
Write-Host '"))'
Write-Host '    gh secret set WIN_SELFSIGN_PFX_BASE64 --repo gordasgdc/CGConvertor --body $b64'
Write-Host '    gh secret set WIN_SELFSIGN_PFX_PASSWORD --repo gordasgdc/CGConvertor'
